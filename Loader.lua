--[[
    💠 XZNE SCRIPTHUB v0.0.01 [Beta] - UNIFIED LOADER
    🚀 Fixes GUI loading issues by embedding UI code directly.
]]

local Repo = "https://raw.githubusercontent.com/AgusSetiawn/TradingMarket-GrowAGarden/perf-v28.1/"
print("[XZNE] Booting v0.0.01 [Beta] (Unified Mode)...")

-- 1. Load Logic Core (Main.lua)
local function LoadLogic()
    local Success, Result = pcall(function()
        return loadstring(game:HttpGet(Repo .. "Main.lua?t=" .. tostring(os.time())))()
    end)
    if not Success then warn("[XZNE] Failed to load Main.lua: " .. tostring(Result)) end
end
...
if not Controller then
    warn("❌ [XZNE] Controller Failed to Load! Check Main.lua error.")
    return
end

print("✅ [XZNE] Logic Core Loaded. initializing GUI...")

-- 3. EMBEDDED GUI CODE (No External Loading = No Cache/Nil Errors)
task.spawn(function()
    -- [LOAD WINDUI LIB]
    local WindUI, Err = loadstring(game:HttpGet("https://raw.githubusercontent.com/Footagesus/WindUI/main/dist/main.lua"))()
    if not WindUI then warn("[XZNE] Failed to load WindUI Lib!"); return end
    ...
    task.spawn(function()
        ...
        WindUI:Notify({ Title = "XZNE v0.0.01 Beta", Content = "Loaded Correctly! (Unified)", Icon = "xzne:check", Duration = 5 })
    end)
end)
